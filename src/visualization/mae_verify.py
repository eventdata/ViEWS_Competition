...
for i in range(445, 480):
    ...
    pred_val = pred_480_fal['log_fatalities'].values
    gt_val = gt_480_fal['ln_ged_best_sb'].values

    mse = ((pred_val-gt_val)**2).mean()
    rmse = np.sqrt(np.mean(pred_val-gt_val)**2)
    mae = (np.abs(pred_val-gt_val)).mean()
    print('MAE:', mae, 'MSE:', mse, 'RMSE:',rmse)


# df = pd.DataFrame(data=pred_val, index=list(range(10677)), columns='pred')
# df['gt'] = gt_val
